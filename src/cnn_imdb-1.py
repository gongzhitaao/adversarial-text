...
with tf.variable_scope('model'):
    env.x = tf.placeholder(tf.float32, [None, 400, 300], 'x')
...
with Timer(msg='\nLoading data'):
    imdb = os.path.expanduser('~/data/imdb/imdb.npz')
    d = np.load(imdb)
    X_train, y_train = d['X_train'], d['y_train']
    X_test, y_test = d['X_test'], d['y_test']

    y_train = 2*y_train.astype(np.float32) - 1
    y_test = 2*y_test.astype(np.float32) - 1
