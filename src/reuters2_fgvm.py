...
with tf.variable_scope('model', reuse=True):
    ...
    env.xadv = fgm(model_fn, env.x, epochs=env.adv_epochs, eps=env.adv_eps,
                   sign=False, clip_min=-100, clip_max=100)
...
X_adv = make_fgsm(sess, env, X_test, eps=15, epochs=5)
...
with open('out/reuters2_fgvm.txt', 'w') as w:
    w.write('\n'.join(X_succ))
...
with open('out/reuters2_fgvm_fail_50.txt', 'w') as w:
    w.write('\n'.join(X_fail))
