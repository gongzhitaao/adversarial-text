...
with tf.variable_scope('model'):
    env.x = tf.placeholder(tf.float32, [None, 160, 300], 'x')
...
X_noise = make_deepfool(sess, env, X_test, epochs=10)

# X_rnd = np.random.random(X_noise.shape) * 2 - 1
X_adv = X_test + 30*X_noise
...
with open('out/reuters2_deepfool.txt', 'w') as w:
    w.write('\n'.join(X_succ))
...
with open('out/reuters2_deepfool_fail_50.txt', 'w') as w:
    w.write('\n'.join(X_fail))

succ = np.logical_and(z0 == z1, z1 != z2).flatten()
tmp = np.vstack((lens, succ)).T
np.save('out/reuters2_deepfool_len-succ.npy', tmp)
